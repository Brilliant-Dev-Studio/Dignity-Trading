...
const DotGrid: React.FC<DotGridProps> = ({
  ...
  const buildGrid = useCallback(() => {
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        dots.push({ cx, cy, xOffset: 0, yOffset: 0, _busy: false });
      }
    }
  ...
  }, [proximity, baseColor, activeRgb, baseRgb, circlePath, baseRgb.r, baseRgb.g, baseRgb.b, activeRgb.r, activeRgb.g, activeRgb.b]);
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (speed > speedTrigger && dist < proximity && !dot._busy) {
          dot._busy = true;
          gsap.killTweensOf(dot);

          const falloff = Math.max(0, 1 - dist / proximity);
          const pushX = (dot.cx - pr.x + vx * 0.004) * 0.18 * falloff;
          const pushY = (dot.cy - pr.y + vy * 0.004) * 0.18 * falloff;
    ...
    const onClick = (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (dist < shockRadius && !dot._busy) {
          ...
          const pushX = (dot.cx - cx) * shockStrength * falloff * 0.035;
          const pushY = (dot.cy - cy) * shockStrength * falloff * 0.035;
  ...
  return (
    <div className={`${styles.dotGrid} ${className}`.trim()} style={style}>
