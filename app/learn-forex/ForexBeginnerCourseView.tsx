...
import { getForexFreeBeginnerCourseForPublic } from "@/lib/public-course";
...
export default async function ForexBeginnerCourseView() {
  const course = await getForexFreeBeginnerCourseForPublic();
  ...
  const displayTitle = course.title.trim() || defaultForexFreeBeginnerTitle;
  const displayEyebrow = course.eyebrow.trim() || defaultForexFreeBeginnerEyebrow;
      ...
      <section className="mt-14">
        ...
        {lessons.length > 0 ? (
          ...
          <p className="mt-4 text-sm text-white/55">
            No lessons yet. Add video links in Admin → Free Forex course (beginner).
          </p>
        )}
      </section>
